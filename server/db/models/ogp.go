...
type Ogp struct {
	...
	Bookmarks   uint      `boil:"bookmarks" json:"bookmarks" toml:"bookmarks" yaml:"bookmarks"`
...
var OgpWhere = struct {
	...
	ID:          whereHelperuint{field: "`ogp`.`id`"},
	UserID:      whereHelperuint{field: "`ogp`.`user_id`"},
	CreatedAt:   whereHelpertime_Time{field: "`ogp`.`created_at`"},
	UpdatedAt:   whereHelpertime_Time{field: "`ogp`.`updated_at`"},
	Bookmarks:   whereHelperuint{field: "`ogp`.`bookmarks`"},
	Type:        whereHelperstring{field: "`ogp`.`type`"},
	URL:         whereHelperstring{field: "`ogp`.`url`"},
	Title:       whereHelperstring{field: "`ogp`.`title`"},
	Description: whereHelperstring{field: "`ogp`.`description`"},
	Image:       whereHelperstring{field: "`ogp`.`image`"},
}
...
var (
	ogpAllColumns            = []string{"id", "user_id", "created_at", "updated_at", "bookmarks", "type", "url", "title", "description", "image"}
	ogpColumnsWithoutDefault = []string{"user_id", "created_at", "bookmarks", "type", "url", "title", "description", "image"}
	ogpColumnsWithDefault    = []string{"id", "updated_at"}
	ogpPrimaryKeyColumns     = []string{"id"}
)
...
func FindOgp(ctx context.Context, exec boil.ContextExecutor, iD uint, selectCols ...string) (*Ogp, error) {
	ogpObj := &Ogp{}
	...
	query := fmt.Sprintf(
		"select %s from `ogp` where `id`=?", sel,
	)
...
func (o *Ogp) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.ID = uint(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == ogpMapping["id"] {
		goto CacheNoHooks
	}
...
func OgpExists(ctx context.Context, exec boil.ContextExecutor, iD uint) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `ogp` where `id`=? limit 1)"
