...
func testBookmarksExists(t *testing.T) {
	...
	e, err := BookmarkExists(ctx, tx, o.UserID, o.FQDN)
	if err != nil {
...
func testBookmarksFind(t *testing.T) {
	...
	bookmarkFound, err := FindBookmark(ctx, tx, o.UserID, o.FQDN)
	if err != nil {
		t.Error(err)
	}
...
var (
	bookmarkDBTypes = map[string]string{`UserID`: `bigint`, `FQDN`: `varchar`, `CreatedAt`: `datetime`}
	_               = bytes.MinRead
)
...
func testBookmarksUpdate(t *testing.T) {
	...
	if rowsAff, err := o.Update(ctx, tx, boil.Whitelist(BookmarkColumns.CreatedAt)); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
